...
import { enableCssTheme } from '../services/theme-installer';
...
export class ThemeInstallEnablePromptModal extends Modal {
	private enabling = false;
	...
	onOpen(): void {
		this.titleEl.setText('Enable theme?');
		this.modalEl.addClass('santi-enable-prompt-modal');
		this.contentEl.empty();
		this.contentEl.addClass('santi-enable-prompt-content');
		...
		setIcon(iconWrap.createSpan({ attr: { 'aria-hidden': 'true' } }), 'palette');

		this.contentEl.createEl('p', {
			cls: 'santi-enable-prompt-lead',
			text: `${this.themeName} is installed in this vault.`,
		});
		this.contentEl.createEl('p', {
			cls: 'santi-enable-prompt-question',
			text: 'Do you also want to enable it now?',
		...
		this.contentEl.createEl('p', {
			cls: 'santi-enable-prompt-hint',
			text: 'It should appear under settings → appearance. You can turn it on later from there.',
		});
		...
		enableBtn.setAttribute('type', 'button');
		enableBtn.addEventListener('click', () => {
			void this.enableTheme(enableBtn, actions);
		});
	}
	...
	private async enableTheme(
		...
		try {
			const result = await enableCssTheme(this.app, this.cssThemeName);

			if (result.enabled) {
				if (result.requiresReload) {
					new Notice(
						`${this.themeName} is set as your theme. Reload Obsidian to apply it.`,
					);
					this.setEnablingState(false);
					this.showReloadAction(actions);
					return;
				...
				new Notice(
					`${this.themeName} is enabled. Check settings → appearance if you do not see it yet.`,
				);
				this.close();
				return;
			}

			throw new Error('Obsidian did not enable the theme.');
		} catch (error) {
			const detail =
				error instanceof Error ? error.message : String(error);
			new Notice(
				`Could not enable ${this.themeName}: ${detail}`,
				8000,
			);
			this.setEnablingState(false);
			enableBtn.disabled = false;
